<template>
	<view>
		...
		<view class="text-center bg-white" v-if="!nodata">
			<view class="flex text-bold padding-tb">
				<view class="flex-sub">Time</view>
				<view class="flex-sub">Amount</view>
			...
			<view class="flex text-sm padding-tb-sm" v-for="(row,i) in datas" :key="i">
				...
				<view class="flex-sub" :style="{color:row.money>0?'#18B566':'#DD514C'}">{{row.money}}</view>
				<view class="flex-sub">{{row.type}}</view>
			</view>
		</view>
		<!-- 没有更多了 -->
	...
	export default {
		...
		methods: {
			loadData(){
				...
				_this.helper.post('account/logs',{page:_this.p}, function(res) {
					var data = res.data;
					var list  = res.data;
